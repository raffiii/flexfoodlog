...
function setupPersistence(elmApp) {
  ...
  const dbReady = initDB().then((d) => db = d).catch(err => {
  ...
  if (elmApp.ports.hydrateStreamCmd) {
    elmApp.ports.hydrateStreamCmd.subscribe(async (info) => {
      await dbReady;
  ...
  if (elmApp.ports.persistEventCmd) {
    elmApp.ports.persistEventCmd.subscribe(async (envelope) => {
  ...
  if (elmApp.ports.persistNewEventCmd) {
    elmApp.ports.persistNewEventCmd.subscribe(async ({ streamType, envelope }) => {
